...
<body>
	...
	%>
		<table border=1>
			<tr><th>编号</th><th>用户名</th><th>密码</th><th>用户类型</th><th>性别</th><th>电话</th><th>邮箱</th><th>住址</th><th>数据操作</th></tr>
			<%
				while (iter.hasNext())
				{
					Yhgl yh = iter.next();
				%>
				<tr><td><%=yh.getId()%></td><td><%=yh.getUsername()%></td><td><%=yh.getPassword() %></td><td><%=yh.getType()%></td><td><%=yh.getSex()%></td><td><%=yh.getPhone()%></td><td><%=yh.getYouxiang()%></td><td><%=yh.getAddress()%></td><td>&nbsp;&nbsp;<a href="sql_update1.jsp?id=<%=yh.getId()%>">修改</a>&nbsp;&nbsp;<a href="sql_delete.jsp?id=<%=yh.getId() %>">删除</a></td></tr>
				<%--通过?传id值 --%>
