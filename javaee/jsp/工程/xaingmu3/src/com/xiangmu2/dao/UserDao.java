...
public class UserDao {
	...
	public int Update(Yhgl yh)
	{
		...
		try
		{
			String sql = "update htgl set username=?,password=?,phone=?,address=?,youxiang=?,type=?,sex=? where id=?";
			pst = c.prepareStatement(sql);
			...
			pst.setString(7, yh.getSex());
			pst.setString(8, yh.getId());
			rs = pst.executeUpdate();
	...
	public int Updatep(Yhgl yh)
	{
		...
		try
		{
			String sql = "update htgl set username=?,password=?,phone=?,address=?,youxiang=?,sex=? where id=?";
			pst = c.prepareStatement(sql);
			pst.setString(1, yh.getUsername());
			pst.setString(2, yh.getPassword());
			pst.setString(3, yh.getPhone());
	...
	public List<Yhgl> Selectmy(int page)//这里要有参数，将页数传过来
	{
		...
		try
		{
			String sql = "select * from htgl limit ?,?";//第一个？表示从第几个记录开始查询，第二个？表示每页几条记录
			pst = c.prepareStatement(sql);
			pst.setInt(1, (page-1)*Yhgl.page_size);//这里都是int类型,直接用类名访问静态成员
			pst.setInt(2, Yhgl.page_size);     
	...
	public ResultSet Panduan(Yhgl yh)
	{	
		ResultSet rs = null;
